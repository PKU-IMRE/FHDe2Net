...
class Scale(object):
  ...
  """
  def __init__(self, size_h, size_w, interpolation=Image.BILINEAR):
    self.size_h = size_h
    self.size_w = size_w
...
class CenterCrop(object):
  ...
  def __init__(self, size_h, size_w):
    if isinstance(size_h, numbers.Number):
      self.size = (int(size_h), int(size_w))
    else:
      self.size = size
...
class RandomCrop(object):
  ...
  or an integer, in which case the target will be of a square shape (size, size)
  """
  def __init__(self, size_h, size_w, padding=0):
    if isinstance(size_h, numbers.Number):
      self.size = (int(size_h), int(size_w))
    else:
      self.size = size
  ...
  def __call__(self, imgA, imgB):
    ...
    for img in imgs:
      ...
      if x1 == -1 and y1 == -1:
        ...
        y1 = random.randint(0, h - th)
      output.append(img.crop((x1, y1, x1 + tw, y1 + th)))
    return output[0], output[1], x1, y1


class GuideCrop(object):
  """Crops the given PIL.Image at a random location to have a region of
  the given size. size can be a tuple (target_height, target_width)
  or an integer, in which case the target will be of a square shape (size, size)
  """
  ...
  def __call__(self,  x1, y1, imgA, imgB):
    imgs = [imgA, imgB]
    output = []
    for img in imgs:
      if self.padding > 0:
...
class RandomHorizontalFlip_index(object):
  """Randomly horizontally flips the given PIL.Image with a probability of 0.5
  """
  def __call__(self, imgA, imgB):
    imgs = [imgA, imgB]
...
class RandomHorizontalFlip4(object):
  """Randomly horizontally flips the given PIL.Image with a probability of 0.5
  """
  def __call__(self, imgA, imgB, imgC, imgD):
    imgs = [imgA, imgB, imgC, imgD]
